   ...
   private void loadData(int requestId) {
      ...
      for (Map<String, Object> artigo : artigos) {
         ...
         if (idFamilia == requestId) {
            ...
            int qtdStock = (int) Double.parseDouble(artigo.get("qtdDisponivel").toString());
            ...
            if (!artigosMap.containsKey(id) || valorLinhaPreco == linhaPrecoId) {
               artigosMap.put(id, new Artigos(id, articleName, price, taxa, qtdStock));
            }
         }
      }
      ...
      articleTable.setOnMouseClicked(event -> {
         if (event.getClickCount() == 1) {
            ...
            if (selectedArticle != null) {
               ...
               addToCheckoutList(idArtigoSelecionado, articleName, price, Double.parseDouble(taxa));
            }
         }
      });
   }

   private void addToCheckoutList(int idArtigoSelecionado, String nomeArtigo, String precoArtigo, double taxa) {
      // CheckoutController checkoutController = CheckoutController.getInstance();
      checkoutList = new ArrayList<>();
      checkoutList.add(new Checkout(idArtigoSelecionado, nomeArtigo, 1, precoArtigo, taxa, qtdArtigoSelecionado));
   }
